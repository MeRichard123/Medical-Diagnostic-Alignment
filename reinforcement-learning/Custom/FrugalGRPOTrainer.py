...
class FrugalGRPOTrainer:
    ...
    def acc(self, input_ids: torch.Tensor, ground_truth=None, text=None
        ):
        ...
        for pred, gt in zip(text, ground_truth):
            ...
            if isinstance(similarity, torch.Tensor):
                accs.append(float(similarity.detach().cpu().item()))
            else:
                accs.append(float(similarity))
    ...
    def score(self, text, ground_truth, input_ids=None):
        ...
        if is_reward_function(self.reward_funcs[0]):
            return torch.tensor([
                np.mean([reward_func(prompt, text) for reward_func in self.reward_funcs])
                for prompt, text in zip(text, ground_truth)
            ], dtype=torch.float32, device=input_ids.device if input_ids is not None else 'cpu')
    ...
    def reliab(self, input_ids: torch.Tensor, prompt=None):
        ...
        return torch.tensor(reliab_scores, device=input_ids.device, 
                            dtype=torch.float32)

    def miscalib(self, input_ids: torch.Tensor, 
                 mask: torch.Tensor,
                 ground_truth=None, text=None,
                 logprobs=None):
        ...
        similarity = cos(gt_embedding, pred_embedding)
        acc = (similarity.item() + 1) / 2  # Scale cosine similarity to [0, 1]

        confidences = masked_mean(logprobs, mask, dim=1)
        confidence = torch.exp(confidences)  # Convert logprobs to probabilities
        
        miscalib = -torch.abs(confidence - acc)  # Higher if confidence is far from accuracy
        return miscalib
    ...
    def U_minus(self, input_ids: torch.Tensor, mask: torch.Tensor, ground_truth=None, text=None, prompt=None, logprobs=None) -> torch.Tensor:
        ...
        reliability_contrib = reliability_score / self._positive_temp(self.t_reliab)
        miscalib_contrib = miscalib_score / self._positive_temp(self.t_miscalib)
        Debug(f"Reliability contrib: {reliability_contrib.mean().item():.4f}, Miscalibration contrib: {miscalib_contrib.mean().item():.4f}")
        
        delta_minus = (
            reliability_contrib + miscalib_contrib
        )
        Debug(f"Delta minus: {delta_minus.mean().item():.4f}")
        return delta_minus.mean().item()
    ...
    def __compute_grpo_advantages(
        ...
        with torch.no_grad():
            ...
            group_mean =  adjusted_utilities.mean(dim=1, keepdim=True)  # (num_prompts, 1)
            # add extra variance using unbiased=False to prevent very small std when group_size is small
            group_std = adjusted_utilities.std(dim=1, keepdim=True, unbiased=False) # (num_prompts, 1) 

            # normalize rewards within each prompt groups
            advantages = (adjusted_utilities - group_mean) / (group_std + 1e-8)  # (num_prompts, group_size)
    ...
    def __compute_reward_tensor(self, input_ids: torch.Tensor, mask: torch.Tensor, full_decoded_texts: List[str], 
                                expanded_ground_truths: List[str], logprobs: torch.Tensor) -> torch.Tensor:
        
        # Compute rewards using the utility functions
        total_rewards = []
        torch.cuda.empty_cache()

        for text, gt in zip(full_decoded_texts, expanded_ground_truths):
            u_plus = self.U_plus(input_ids=input_ids, ground_truth=gt, text=text, prompt=text)
            u_minus = self.U_minus(input_ids=input_ids, mask=mask, ground_truth=gt, text=text, prompt=text, logprobs=logprobs)
            Debug(f"U_plus: {u_plus:.4f}, U_minus: {u_minus:.4f}")
            utility = u_plus - u_minus
            Debug(f"Utility: {utility:.4f}")
            total_rewards.append(utility)
        Debug(f"Total rewards: {total_rewards}")
        return torch.Tensor(total_rewards).detach().float()

...
class MORLGRPOTrainerVL:
    def __init__(self, args: GRPOConfig, reward_funcs: List[RewardFunc] = [], reward_processors: List[Optional[PreTrainedTokenizerBase]] = []):
        self.args = args
        ...
        self.ema_deficiency = None
        self.deficiency_momentum = 0.9
        self.pareto_update_count = 0
        self.pareto_warm_up_steps = 10
        self.util_run_mean = None
        self.util_run_std = None
        self.util_norm_momentum = 0.95
        ...
        self.model, self.ref_model,self.tokenizer,self.processor = self.__load_models_and_tokenizer(
            self.device
        )

        self.weights = torch.ones(self.num_components, device=self.device) 
    ...
    @staticmethod
    def _repeat_visual_inputs(
        pixel_values: Optional[torch.Tensor],
        image_grid_thw: Optional[torch.Tensor],
        repeats: int,
    ) -> Tuple[Optional[torch.Tensor], Optional[torch.Tensor]]:
        """Repeat flattened image patches while preserving each image's patch block."""
        if pixel_values is None or image_grid_thw is None or repeats == 1:
            return pixel_values, image_grid_thw

        grids = image_grid_thw.reshape(-1, 3)
        patch_counts = (grids[:, 0] * grids[:, 1] * grids[:, 2]).to(torch.long)
        if int(patch_counts.sum()) != pixel_values.shape[0]:
            raise ValueError(
                "pixel_values and image_grid_thw are inconsistent: "
                f"{pixel_values.shape[0]} patches for {int(patch_counts.sum())} grid patches."
            )

        patch_blocks = torch.split(pixel_values, patch_counts.tolist(), dim=0)
        repeated_pixels = torch.cat(
            [block for block in patch_blocks for _ in range(repeats)], dim=0
        )
        repeated_grids = image_grid_thw.repeat_interleave(repeats, dim=0)
        return repeated_pixels, repeated_grids
    
    @staticmethod
    def _select_visual_inputs(
        pixel_values: Optional[torch.Tensor],
        image_grid_thw: Optional[torch.Tensor],
        start: int,
        end: int,
    ) -> Tuple[Optional[torch.Tensor], Optional[torch.Tensor]]:
        """Select complete per-image patch blocks for a language-model batch slice."""
        ...
        selected_counts = patch_counts[start:end]
        pixel_start = int(patch_counts[:start].sum())
        pixel_end = pixel_start + int(selected_counts.sum())
        return pixel_values[pixel_start:pixel_end], image_grid_thw[start:end]
    ...
    def score(self, text, ground_truth, input_ids=None) -> torch.Tensor | None:
        """
        Returns a tensor of shape (batch_size, num_reward_funcs + 1)
        where each row contains [r1, r2, ..., rn, acc] for that sample.
        """
        if isinstance(text, str):
            text = [text]
        if isinstance(ground_truth, str):
        ...
        # ---- 1. Get per‑reward‑function values (as a matrix) ----
        if is_reward_function(self.reward_funcs[0]):
            # Scalar reward functions – we need to call each for every sample
            reward_matrix = []
            for prompt, gt in zip(text, ground_truth):
                sample_vals = []
                for rf in self.reward_funcs:
                    try:
                        val = float(rf(prompt, gt))
                    except Exception:
                        val = 0.0
                    sample_vals.append(val)
                reward_matrix.append(sample_vals)
            rewards = torch.tensor(reward_matrix, dtype=torch.float32, device=device)
            return rewards
        
        elif is_reward_model(self.reward_funcs[0]):
            # Reward models handle batches – we collect outputs from each model
            rewards_list = []
            for rf, processor in zip(self.reward_funcs, self.reward_processors):
                # (same input preparation as in your original score)
                if processor is not None:
                    if processor.pad_token is None:
                        processor.pad_token = processor.eos_token
                        processor.pad_token_id = processor.eos_token_id
                    rf.config.pad_token_id = processor.pad_token_id
                    inputs = processor(text, padding=True, return_tensors='pt', truncation=True)
                    inputs = {k: v.to(device) for k, v in inputs.items()}
                else:
                    inputs = {'input_ids': torch.tensor([0], device=device)}
                
                rf = rf.to(device)
                with torch.no_grad():
                    outputs = rf(**inputs)
                    r = outputs.logits.squeeze(-1) if hasattr(outputs, "logits") else outputs.squeeze(-1)
            ...
            rewards = torch.stack(rewards_list, dim=1)   # (batch_size, num_models)
            return rewards
    ...
    def miscalib(self, input_ids: torch.Tensor, mask: torch.Tensor,
        ...
        confidence = self._confidence_from_logprobs(mask, logprobs).to(input_ids.device)
        miscalib_scores = torch.abs(confidence - accuracy)
        return miscalib_scores
    ...
    def _update_ideal_point(self, utilities: torch.Tensor) -> None:
        """utilities: (N, num_components), higher = better. Tracks a running best-seen
        point so the Tchebycheff scalarization always has a valid utopia reference."""
        with torch.no_grad():
            batch_best = utilities.max(dim=0).values.detach()
            if self.ideal_point is None:
                self.ideal_point = batch_best
            else:
                self.ideal_point = torch.maximum(self.ideal_point, batch_best)

    def _pareto_weights(self, utilities: torch.Tensor, eta: float = 0.5) -> torch.Tensor:
        """Adaptive Tchebycheff weights via EMA-smoothed deficiency: whichever
        component is furthest below its ideal point (on average, over recent steps)
        gets upweighted. EMA keeps the weights from whipping around on noisy
        small-group rollouts while still tracking genuine drift in which
        objective is lagging."""
        if self.ideal_point is None:
            self.ideal_point = utilities.max(dim=0).values.detach()
        with torch.no_grad():
            num_components = utilities.shape[1]
            mean_utils = utilities.mean(dim=0)
            raw_deficiency = (self.ideal_point - mean_utils).clamp(min=0)

            if self.ema_deficiency is None:
                self.ema_deficiency = raw_deficiency
            else:
                m = self.deficiency_momentum
                self.ema_deficiency = m * self.ema_deficiency + (1 - m) * raw_deficiency

            self.pareto_update_count += 1

            if self.pareto_update_count <= self.pareto_warm_up_steps:
                return torch.full(
                    (num_components,), 1.0 / num_components,
                    device=utilities.device, dtype=utilities.dtype
                )

            deficiency = self.ema_deficiency / (self.ema_deficiency.sum() + 1e-8)
            w = torch.softmax(eta * deficiency / self._positive_temp(self.weights[:num_components]), dim=0)
        return w
    
    def _pareto_scalarize(self, utilities: torch.Tensor, w: torch.Tensor, augment_eps: float = 0.05) -> torch.Tensor:
        """Augmented weighted Tchebycheff scalarization: for ANY w > 0 the maximizer
        of this is Pareto optimal, including on non-convex regions of the front
        (unlike w @ utilities, which only reaches the convex hull)."""
        if self.ideal_point is None:
            self.ideal_point = utilities.max(dim=0).values.detach()

        gap = w.unsqueeze(0) * (self.ideal_point.unsqueeze(0) - utilities)  # (N, C) >= 0
        cheby_term = -gap.max(dim=1).values
        augment_term = augment_eps * (w.unsqueeze(0) * utilities).sum(dim=1)  # breaks ties -> strict Pareto opt
        return cheby_term + augment_term
    ...
    def U_plus(self, input_ids: torch.Tensor, ground_truth=None, text=None, prompt=None) -> torch.Tensor:
        ...
        accuracy = self.acc(input_ids, ground_truth=ground_truth, text=text)
        # accuracy should be (batch_size, R)
        
        acc_contrib = accuracy  # Remove temperature scaling for now
        
        # Compute reward model scores (B, R)
        ...
        if reward_model_score is not None and isinstance(reward_model_score, torch.Tensor):
            ...
            score_agg = reward_scaled.mean(dim=1, keepdim=True) 
            return torch.cat([score_agg, acc_contrib.unsqueeze(1)], dim=1)
        
        return torch.cat([acc_contrib.unsqueeze(1), acc_contrib.unsqueeze(1)], dim=1)  # fallback, keep width=2
    ...
    def U_minus(self, input_ids: torch.Tensor, mask: torch.Tensor, ground_truth=None, 
        ...
        """Returns tensor of shape (batch_size,)"""
        batch_size = input_ids.shape[0] if input_ids.ndim > 1 else 1
        group_size = self.args.group_size if self.args.group_size is not None else 1
        ...
        if group_size is not None and logprobs is not None:
            confidence = self._confidence_from_logprobs(mask, logprobs)
            print(f"Confidence: {confidence.mean().item():.4f}")
            accuracy = self.acc(input_ids, ground_truth=ground_truth, text=text)
            calib_align = self.calibration_alignment(confidence, accuracy).reshape(-1, 1)
            print(f"Calibration alignment: {calib_align.mean().item():.4f}")
            return torch.cat([miscalib_score, calib_align], dim=1)
        
        return torch.cat([miscalib_score, miscalib_score], dim=1)  # fallback, keep width=2

    def _normalize_utilities(self, utilities: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            batch_mean = utilities.mean(dim=0)
            batch_std = utilities.std(dim=0, unbiased=False).clamp(min=1e-3)
            if self.util_run_mean is None or self.util_run_std is None:
                self.util_run_mean, self.util_run_std = batch_mean, batch_std
            else:
                m = self.util_norm_momentum
                self.util_run_mean = m * self.util_run_mean + (1 - m) * batch_mean
                self.util_run_std = m * self.util_run_std + (1 - m) * batch_std
        return (utilities - self.util_run_mean) / (self.util_run_std + 1e-6)
    ...
    def __compute_grpo_advantages(
        ...
        """
        Compute GRPO advantages by combining utilities and KL penalties, then applying GAE.

        Args:
            utilities: Tensor of shape (batch_size, seq_len) containing token-level utilities.
            ...
            adv_eps: Epsilon value for numerical stability in advantage computation.
        Returns:
        ...
        with torch.no_grad():
            ...
            num_components = utilities.shape[1] if utilities.dim() > 1 else 1
            ...
            if utilities.dim() == 1:
                ...
                scalar_utilites  = utilities - self.args.kl_coeff * mean_kl_per_seq  # (num_prompts, group_size)
            else:
                utilities = utilities.view(num_prompts, group_size, num_components)
                mean_kl_per_seq = mean_kl_per_seq.view(num_prompts, group_size).unsqueeze(-1)
                adjusted_utilities = utilities - self.args.kl_coeff * mean_kl_per_seq  # (P, G, C)
                print("Using multi-component utilities for GRPO advantage computation.")

                flat = adjusted_utilities.view(-1, num_components)
                flat = self._normalize_utilities(flat)
                self._update_ideal_point(flat)
                w = self._pareto_weights(flat)
                scalar_utilites = self._pareto_scalarize(flat, w).view(num_prompts, group_size)
        # --- GRPO group-relative normalization ---

        group_mean = scalar_utilites.mean(dim=1, keepdim=True)          # (P, 1)
        if group_size > 1:
            group_std = scalar_utilites.std(dim=1, keepdim=True, unbiased=False)            # (P, 1)
        else:
            group_std = torch.ones_like(group_mean)
        
        group_advantages = (scalar_utilites - group_mean) / (group_std + adv_eps)  # (P, G)

        # --- broadcast per-sample scalar to token level ---
        per_sample_advantages = group_advantages.view(num_samples)      # (N,)
        seq_len = response_mask.shape[1]
        advantages = per_sample_advantages.unsqueeze(1).expand(num_samples, seq_len)  # (N, seq_len)
        advantages = advantages * response_mask  # zero out padding tokens
    ...
    def __compute_grpo_entropy_loss(
        ...
        Returns positive scalar entropy loss (higher = more exploration).
        """
        mask = response_mask.bool()
        
        # Using log_probs (memory efficient)
    ...
    def __generate_responses_grouped(
        ...
        with torch.no_grad():
            ...
            expanded_mm_token_type_ids = (
                None if mm_token_type_ids is None
                else mm_token_type_ids.repeat_interleave(group_size, dim=0)
            )
            ...
            if expanded_mm_token_type_ids is not None:
                gen_kwargs["mm_token_type_ids"] = expanded_mm_token_type_ids
            expanded_pixel_values, expanded_image_grid_thw = self._repeat_visual_inputs(
                pixel_values, image_grid_thw, group_size
            )
            if expanded_pixel_values is not None:
                gen_kwargs["pixel_values"] = expanded_pixel_values
            if expanded_image_grid_thw is not None:
                gen_kwargs["image_grid_thw"] = expanded_image_grid_thw

            generated_output = self.model.generate(**gen_kwargs)
    ...
    def __calculate_rollout_stats(
        ...
        if expanded_mm_token_type_ids is not None:
            full_mm_token_type_ids = torch.cat(
                [expanded_mm_token_type_ids, torch.zeros_like(response_ids)], dim=1
            )
        ...
        with torch.no_grad():
            ...
            for i in range(0, full_ids.shape[0], chunk_size):
                ...
                chunk_mm_token_type_ids = (
                    None if full_mm_token_type_ids is None
                    else full_mm_token_type_ids[i:i+chunk_size]
                )
                chunk_pixel_values, chunk_image_grid_thw = self._select_visual_inputs(
                    expanded_pixel_values,
                    expanded_image_grid_thw,
                    i,
                    i + chunk_size,
                )

                model_kwargs = {"input_ids": chunk_ids, "attention_mask": chunk_mask}
                if chunk_mm_token_type_ids is not None:
                    model_kwargs["mm_token_type_ids"] = chunk_mm_token_type_ids
                if chunk_pixel_values is not None:
                    model_kwargs["pixel_values"] = chunk_pixel_values
                if chunk_image_grid_thw is not None:
                    model_kwargs["image_grid_thw"] = chunk_image_grid_thw

                logits = self.model(**model_kwargs).logits  # (chunk_size, seq_len, vocab_size)
                ...
                if start_idx < 0 or end_idx <= start_idx or resp_len == 0:
                    ...
                    if self.ref_model:
                        ref_kwargs = {"input_ids": chunk_ids, "attention_mask": chunk_mask}
                        if chunk_mm_token_type_ids is not None:
                            ref_kwargs["mm_token_type_ids"] = chunk_mm_token_type_ids
                        if chunk_pixel_values is not None:
                            ref_kwargs["pixel_values"] = chunk_pixel_values
                        if chunk_image_grid_thw is not None:
                            ref_kwargs["image_grid_thw"] = chunk_image_grid_thw
    ...
    def __compute_reward_tensor(self, input_ids: torch.Tensor, mask: torch.Tensor, 
        ...
        utils = torch.cat([u_plus, u_minus], dim=1)  # Shape: (batch_size, R)
        if torch.isnan(utils).any():
            print("⚠️ WARNING: NaN detected in utilities!")
            utils = torch.nan_to_num(utils, nan=0.0)
    ...
    def __compute_rollout(
        ...
        for batch in progress_bar:
            ...
            if mm_token_type_ids is not None:
                buffer_lists.setdefault('mm_token_type_ids', []).append(mm_token_type_ids.cpu())
            if pixel_values is not None:
                buffer_lists.setdefault('pixel_values', []).append(pixel_values.cpu())
            if image_grid_thw is not None:
                buffer_lists.setdefault('image_grid_thw', []).append(image_grid_thw.cpu())
        ...
        if 'mm_token_type_ids' in buffer_lists:
            collated_buffer['mm_token_type_ids'] = pad_and_collate_tensors(
                buffer_lists['mm_token_type_ids'], 0
            )
        if 'pixel_values' in buffer_lists and buffer_lists['pixel_values']:
            visual_inputs = []
            for batch_pixel_values, batch_image_grid_thw in zip(
                buffer_lists['pixel_values'], buffer_lists['image_grid_thw']
            ):
                grids = batch_image_grid_thw.reshape(-1, 3)
                patch_counts = (
                    grids[:, 0] * grids[:, 1] * grids[:, 2]
                ).to(torch.long)
                pixel_blocks = torch.split(
                    batch_pixel_values, patch_counts.tolist(), dim=0
                )
                visual_inputs.extend(
                    (pixels, grid.unsqueeze(0))
                    for pixels, grid in zip(pixel_blocks, grids)
                )
            collated_buffer['visual_inputs'] = visual_inputs
    ...
    def __run_grpo_update_epoch(
        ...
        prompt_mm_token_type_ids = collated_buffer.get('mm_token_type_ids')
        ...
        for i in range(0, num_prompts, self.args.mini_batch_size):
            ...
            fwd_prompt_mm_token_type_ids = None
            if prompt_mm_token_type_ids is not None:
                fwd_prompt_mm_token_type_ids = prompt_mm_token_type_ids[
                    prompt_batch_indicies
                ].repeat_interleave(group_size, dim=0).to("cpu")
            batch_pixel_values = None
            batch_image_grid_thw = None
            if collated_buffer.get('visual_inputs') is not None:
                selected_visual_inputs = [
                    collated_buffer['visual_inputs'][index]
                    for index in prompt_batch_indicies
                ]
                batch_pixel_values = torch.cat(
                    [pixels for pixels, _ in selected_visual_inputs], dim=0
                )
                batch_image_grid_thw = torch.cat(
                    [grid for _, grid in selected_visual_inputs], dim=0
                )
            if batch_pixel_values is not None:
                batch_pixel_values, batch_image_grid_thw = self._repeat_visual_inputs(
                    batch_pixel_values.to(device),
                    batch_image_grid_thw.to(device),
                    group_size,
                )
            ...
            batch_full_mm_token_type_ids = None
            if fwd_prompt_mm_token_type_ids is not None:
                batch_full_mm_token_type_ids = torch.cat(
                    [
                        fwd_prompt_mm_token_type_ids,
                        torch.zeros_like(batch_response_ids),
                    ],
                    dim=1,
                )
            ...
            forward_kwargs = {
                ...
                "attention_mask": batch_full_mask.to(device),
            }
            if batch_full_mm_token_type_ids is not None:
                forward_kwargs["mm_token_type_ids"] = batch_full_mm_token_type_ids.to(device)
            if batch_pixel_values is not None:
                forward_kwargs["pixel_values"] = batch_pixel_values
            if batch_image_grid_thw is not None:
                forward_kwargs["image_grid_thw"] = batch_image_grid_thw

            logits_new = self.model(**forward_kwargs).logits
    ...
    def __load_models_and_tokenizer(
            self,
            device: torch.device
        ) -> Tuple[PreTrainedModel, PreTrainedModel, PreTrainedTokenizerBase, Any]:

        print(f"Loading processor/tokenizer: {self.args.model.tokenizer_name}")
        ...
        actor_model = AutoModelForImageTextToText.from_pretrained(
            self.args.model.model_name, **model_kwargs
        )

        if not model_kwargs.get("quantization_config"):
            actor_model = actor_model.to(device)
        if actor_model.config.image_token_id is None:
            raise ValueError("The loaded VLM configuration does not define image_token_id.")
        ...
        if ref_model and ref_model.config.image_token_id is None:
            raise ValueError("The reference VLM configuration does not define image_token_id.")
        ...
        return actor_model, ref_model, tokenizer, processor
    ...
    def train(self):
        ...
        processed_dataset = load_and_process_dataset(
        ...
        def collate_fn(batch):
            ...
            for item in batch:
                ...
                images.append(item.get('images') or [])
                ground_truths.append(item['solution'])

            if self.processor is not None and any(imgs for imgs in images):
                ...
                return {
                    ...
                    "prompt_attention_mask": processed["attention_mask"],
            ...
            input_ids = [self.tokenizer.encode(prompt, truncation=True, max_length=512) for prompt in prompts]
        ...
        for grpo_step in range(self.args.training.total_ppo_steps):
            ...
            if num_prompts_to_select < rollout_prompts:
                print(f"Warning: Requested {rollout_prompts} prompts for rollout but only {len(processed_dataset)} available. Using all available prompts.")
            
            prompt_dataloader = DataLoader(
                processed_dataset.shuffle(seed=self.args.training.seed + grpo_step).select(range(num_prompts_to_select)),
            ...
            log_data["rollout/acc_mean"] = rewards[:, 1].mean().item()
            log_data["rollout/score_mean"] = rewards[:, 0].mean().item()
            log_data["rollout/miscalib_mean"] = rewards[:, 2].mean().item()
            log_data["rollout/calib_align_mean"] = rewards[:, 3].mean().item()

            if self.args.wandb.report_to_wandb:
                for i, name in enumerate(["score", "acc", "miscalib", "calib_align"]):
                    if self.ideal_point is not None and not self.ema_deficiency is None:
                        log_data[f"pareto/ideal_{name}"] = self.ideal_point[i].item()
                        log_data[f"pareto/deficiency_{name}"] = self.ema_deficiency[i].item()
